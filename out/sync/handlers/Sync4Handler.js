...
class Sync4Handler {
    ...
    static handleEditorToForm(context) {
        if (Sync4Handler.isUpdatingForm || context.isEditorChangeFromExtension || context.isEditingForm) {
            return;
        }
        if (Sync4Handler.debounceTimer) {
            clearTimeout(Sync4Handler.debounceTimer);
        }
        Sync4Handler.debounceTimer = setTimeout(() => {
            Sync4Handler.executeEditorToForm(context);
        }, Sync4Handler.DEBOUNCE_MS);
    }
    static handleFormToEditor(context) {
        if (Sync4Handler.isUpdatingEditor) {
            return;
        }
        if (Sync4Handler.debounceTimer) {
            clearTimeout(Sync4Handler.debounceTimer);
        ...
        Sync4Handler.debounceTimer = setTimeout(() => {
            Sync4Handler.executeFormToEditor(context);
        }, Sync4Handler.DEBOUNCE_MS);
    }
    static executeEditorToForm(context) {
        try {
            Sync4Handler.isUpdatingForm = true;
            ...
            console.log('[Sync4] 编辑器已同步到表单 (表首行=光标行)');
        }
        catch (error) {
            console.error('[Sync4] 编辑器到表单同步失败:', error);
        }
        finally {
            Sync4Handler.isUpdatingForm = false;
        }
    }
    static executeFormToEditor(context) {
        try {
            Sync4Handler.isUpdatingEditor = true;
            const document = context.editor.document;
            ...
            if (columnIndex === -1) {
                console.warn('[Sync4] 列不存在:', context.column);
                return;
            }
            ...
            console.log('[Sync4] 表单已同步到编辑器 (表首行=光标行)');
        }
        catch (error) {
            console.error('[Sync4] 表单到编辑器同步失败:', error);
        }
        finally {
            Sync4Handler.isUpdatingEditor = false;
        }
    }
    static clear() {
        if (Sync4Handler.debounceTimer) {
            clearTimeout(Sync4Handler.debounceTimer);
            Sync4Handler.debounceTimer = null;
        }
        Sync4Handler.isUpdatingForm = false;
        Sync4Handler.isUpdatingEditor = false;
    }
}
exports.Sync4Handler = Sync4Handler;
//# sourceMappingURL=Sync4Handler.js.map